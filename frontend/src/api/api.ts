const BASE_URL = 'http://127.0.0.1:50505'; // Backend URL
...
export const historyList = async (offset = 0): Promise<Conversation[] | null> => {
  const response = await fetch(`${BASE_URL}/history/list?offset=${offset}`, {
    method: 'GET'
  })
    .then(async res => {
...
export const frontendSettings = async (): Promise<Response | null> => {
  const response = await fetch(`${BASE_URL}/frontend_settings`, {
    method: 'GET'
  })
    .then(res => {
      return res.json()
...
export const historyMessageFeedback = async (messageId: string, feedback: string): Promise<Response> => {
  const response = await fetch(`${BASE_URL}/history/message_feedback`, {
    method: 'POST',
    body: JSON.stringify({
